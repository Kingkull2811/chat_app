...
class _VoiceCallPageState extends State<VoiceCallPage> {
  ...
  Widget build(BuildContext context) {
    bool isMe = widget.call.callerId == SharedPreferencesStorage().getUserId().toString() ? true : false;
    return WillPopScope(
      ...
      child: Scaffold(
        body: SafeArea(
          child: ZegoUIKitPrebuiltCall(
            ...
            userID: isMe ? (widget.call.callerId ?? '') : (widget.call.receiverId ?? ''),
            userName: isMe ? (widget.call.callerName ?? '') : (widget.call.receiverName ?? ''),
            callID: widget.call.channelName,
            config: ZegoUIKitPrebuiltCallConfig.oneOnOneVoiceCall()
              ..avatarBuilder = (context, Size size, ZegoUIKitUser? user, Map<String, dynamic> map) {
